...
class PhantomSTT:
    ...
    Features:
    - Zero Memory Leak / Single Persistent Listener Thread Architecture
    - Instant Speech Cutoff (Barge-In) when User Voice Detected
    - High-Sensitivity Energy Threshold (100)
    """
    def __init__(self, wake_word="phantom", energy_threshold=100, on_command_callback=None, tts_engine=None):
        ...
        print("\n[PHANTOM STT] Initializing High-Sensitivity Short-Word Neural Engine...")

    def listen_loop(self):
        """Single Persistent Speech Listening Loop."""
        print("✨ [PHANTOM STT] Online Neural Engine Active! Tuned for Short Commands & Instant Interruption.")

        with sr.Microphone() as source:
            print("[PHANTOM STT] Calibrating ambient room audio...")
            try:
            ...
            while self.is_listening:
                try:
                    # Capture audio input
                    audio = self.recognizer.listen(source, timeout=1.0, phrase_time_limit=4.0)
                    ...
                    # While waiting, check if TTS needs to be cut off
                    continue
